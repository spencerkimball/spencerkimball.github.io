...
Range.prototype.getZoneConfig = function() {
  // If there's a region, lookup zone config in partitioned dictionary.
  if (this.region) {
    if (!(this.region in this.table.zoneConfig)) {
      return this.table.zoneConfig;
    }
    return this.table.zoneConfig[this.region];
  }
  return this.table.zoneConfig;
}

Range.prototype.color = function() {
  return this.table.color(this.region);
}
...
Range.prototype.applyCommand = function(req) {
  this.log.push(req);
  this.flushLog();
  if (req.completionFn != null) {
    req.completionFn(req);
  }
}
...
Range.prototype.maybeSplit = function() {
  ...
  while (true) {
    ...
    if (this.curSplitEpoch == this.nextSplitEpoch && qSize >= this.model.splitSize) {
      ...
      var newReplicas = {};
      for (var i = 0; i < this.replicas.length; i++) {
        var r = new Replica(0, i, newRange, this.replicas[i].roachNode, true, this.model);
...
// setReplica sets the replica at the specified index of the range's replica array.
Range.prototype.setReplica = function(replica, index) {
...
Range.prototype.add = function(req) {
  ...
  if (! ("completed" in req.replicated)) {
    if (failures > (this.replicas.length - this.quorum())) {
      ...
      req.replicated["completed"] = true;
      //console.log(req.id + " succeeded with " + successes + " successes; applying to log")
