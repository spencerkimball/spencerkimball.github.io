...
RoachNode.prototype.usageByTable = function(usageMap, replicasByRangeIDMap) {
  for (var i = 0; i < this.replicas.length; i++) {
    if (this.replicas[i].range != null) {
      ...
      replicasByRangeIDMap[this.replicas[i].range.id] |= (0x1 << this.replicas[i].index);
